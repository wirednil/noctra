...
impl Repl {
    ...
    fn handle_use_source(&mut self, path: &str, alias: Option<&str>, _options: &HashMap<String, String>) -> Result<()> {
        ...
        if path.ends_with(".csv") || path.ends_with(".json") || path.ends_with(".parquet") {
            ...
            eprintln!("[DEBUG] Loading file into DuckDB: {} as {}", path, table_name);
            ...
            if duckdb_source_exists {
                ...
                #[cfg(debug_assertions)]
                eprintln!("[DEBUG] Using existing DuckDB source");
                ...
                if let Some(source) = registry.get_mut("duckdb") {
                    ...
                    #[cfg(debug_assertions)]
                    eprintln!("[DEBUG] File registered in existing DuckDB instance");
                } else {
                    return Err(NoctraError::Internal("DuckDB source not found".to_string()));
                }
            } else {
                // Crear nueva instancia DuckDB y registrarla
                #[cfg(debug_assertions)]
                eprintln!("[DEBUG] Creating new DuckDB source");
                ...
                #[cfg(debug_assertions)]
                eprintln!("[DEBUG] DuckDB source registered as 'duckdb'");
            }

            #[cfg(debug_assertions)]
            eprintln!("[DEBUG] Active source: {:?}",
