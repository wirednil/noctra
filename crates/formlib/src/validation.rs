...
use regex::Regex;
use chrono::{DateTime, NaiveDate, NaiveDateTime, Local};
use thiserror::Error;

use crate::forms::{Form, FormField, FieldValidations, FieldType};
...
impl FormValidator {
    ...
    pub fn validate_form(&self, form: &Form, values: &HashMap<String, String>) -> Result<(), Vec<ValidationError>> {
        ...
        match &field.field_type {
            FieldType::Text => {
                if !value.chars().all(|c| c.is_ascii_graphic() || c.is_whitespace()) {
                    Err(ValidationError::InvalidType(field.label.clone(), "Texto inválido".to_string()))
                } else {
                    Ok(())
                }
            }
            ...
            FieldType::Int => {
                match value.parse::<i64>() {
                    Ok(_) => Ok(()),
                    Err(_) => Err(ValidationError::InvalidType(field.label.clone(), "Entero inválido".to_string()))
                }
            }
            ...
            FieldType::Float => {
                match value.parse::<f64>() {
                    Ok(_) => Ok(()),
                    Err(_) => Err(ValidationError::InvalidType(field.label.clone(), "Flotante inválido".to_string()))
                }
            }
            ...
            FieldType::Boolean => {
                match value.to_lowercase().as_str() {
                    ...
                    _ => Err(ValidationError::InvalidType(field.label.clone(), "Booleano inválido".to_string()))
                }
            }
            ...
            FieldType::Email => {
                if value.contains('@') && value.contains('.') {
                    ...
                    Err(ValidationError::InvalidType(field.label.clone(), "Email inválido".to_string()))
                }
            }
            ...
            FieldType::Date => {
                match NaiveDate::parse_from_str(value, "%Y-%m-%d") {
                    Ok(_) => Ok(()),
                    Err(_) => Err(ValidationError::InvalidType(field.label.clone(), "Fecha inválida (YYYY-MM-DD)".to_string()))
                }
            }
            ...
            FieldType::DateTime => {
                match NaiveDateTime::parse_from_str(value, "%Y-%m-%d %H:%M:%S") {
                    Ok(_) => Ok(()),
                    Err(_) => Err(ValidationError::InvalidType(field.label.clone(), "FechaHora inválida (YYYY-MM-DD HH:MM:SS)".to_string()))
                }
            }
            ...
            FieldType::Select { options } => {
                if options.contains(&value.to_string()) {
                    ...
                    Err(ValidationError::ValueNotAllowed(value.to_string(), field.label.clone()))
                }
            }
            ...
            FieldType::MultiSelect { options, .. } => {
                ...
                for val in values {
                    if !options.contains(&val.to_string()) {
                        return Err(ValidationError::ValueNotAllowed(val.to_string(), field.label.clone()));
                    }
                }
                Ok(())
            }
    ...
    fn validate_field_validations(&self, field: &FormField, value: &str, validations: &FieldValidations) -> ValidationResult {
        ...
        if let Some(min_length) = validations.min_length {
            if value.len() < min_length {
                return Err(ValidationError::InvalidLength(field.label.clone(), 
        ...
        if let Some(allowed_values) = &validations.allowed_values {
            if !allowed_values.contains(&value.to_string()) {
                return Err(ValidationError::ValueNotAllowed(value.to_string(), field.label.clone()));
            }
        }
        ...
        match &field.field_type {
            FieldType::Int => {
                ...
                let min_val: i64 = min.parse().map_err(|_| 
                    ValidationError::InvalidType(field.label.clone(), "Valor mínimo inválido".to_string()))?;
                
                if val < min_val {
                    Err(ValidationError::OutOfRange(field.label.clone(), 
            ...
            FieldType::Float => {
                let val: f64 = value.parse().map_err(|_| 
                    ValidationError::InvalidType(field.label.clone(), "Flotante inválido".to_string()))?;
        ...
        match &field.field_type {
            FieldType::Int => {
                ...
                let max_val: i64 = max.parse().map_err(|_| 
                    ValidationError::InvalidType(field.label.clone(), "Valor máximo inválido".to_string()))?;
        ...
        match Regex::new(pattern) {
            Ok(regex) => {
                if regex.is_match(value) {
                    ...
                    Err(ValidationError::PatternMismatch(field.label.clone(), 
                        format!("No cumple el patrón: {}", pattern)))
                }
            }
            Err(_) => {
                Err(ValidationError::InvalidType(field.label.clone(), "Patrón regex inválido".to_string()))
            }
        }
    }
}