...
use crate::types::{Value, SessionVariables, Parameters};
...
impl Session {
    ...
    {
        Ok(self.get_variable(name).and_then(|v| {
            match v {
...
impl SessionManager {
    ...
    pub fn create_session(&mut self) -> Result<Session> {
        ...
        if self.sessions.len() >= self.config.max_sessions {
            return Err(NoctraError::Configuration(
                format!("Máximo de sesiones alcanzado: {}", self.config.max_sessions)
            ));
        }
    ...
    pub fn cleanup_finished_sessions(&mut self) {
        let finished: Vec<String> = self.sessions.iter()
