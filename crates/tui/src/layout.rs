...
impl LayoutManager {
    ...
    pub fn add_element_with_rect<T: Component + 'static>(
        &mut self,
        id: String,
        component: T,
        rect: Rect,
    ...
    pub fn recalculate_layout(&mut self) {
        ...
        match strategy {
            ...
            LayoutStrategy::Weighted {
                ref vertical_weights,
                ref horizontal_weights,
                spacing,
                padding,
            } => {
    ...
    fn apply_grid_layout(
        ...
        let cell_width = (self
            ...
            .size
            .width
            .saturating_sub(padding * 2 + column_spacing * (columns - 1)))
            / columns;
        let cell_height = (self
            .total_area
            .size
            .height
            .saturating_sub(padding * 2 + row_spacing * (rows - 1)))
            / rows;
        ...
        for element in &mut self.elements {
            ...
            // Calcular posición
            element.rect.position.x =
                self.total_area.position.x + padding + col * (cell_width + column_spacing);
            element.rect.position.y =
                self.total_area.position.y + padding + row * (cell_height + row_spacing);
