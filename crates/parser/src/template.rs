...
impl TemplateEngine {
    ...
    pub fn process(&self, template: &str, variables: &HashMap<String, String>) -> ParserResult<String> {
    ...
    /// Procesar condicionales simples
    fn process_conditionals(&self, template: &str, variables: &HashMap<String, String>) -> ParserResult<String> {
        let mut result = template.to_string();
        
        // {{#if variable}} ... {{/if}}
        let if_regex = Regex::new(r"\{\{#if\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}(.*?)\{\{/if\}\}").unwrap();
        ...
        let unless_regex = Regex::new(r"\{\{#unless\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}(.*?)\{\{/unless\}\}").unwrap();
    ...
    /// Procesar loops simples
    fn process_loops(&self, template: &str, variables: &HashMap<String, String>) -> ParserResult<String> {
        let mut result = template.to_string();
        
        // {{#each array}} ... {{/each}}
        let each_regex = Regex::new(r"\{\{#each\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}(.*?)\{\{/each\}\}").unwrap();
    ...
    pub fn validate_template(&self, template: &str) -> ParserResult<()> {
        ...
        if open_count != close_count {
            return Err(ParserError::template_error("Unbalanced template delimiters"));
        }
...
pub mod utils {
    ...
    pub fn process_sql_template(template: &str, variables: &HashMap<String, String>) -> String {
        ...
        engine.process(template, &session_vars).unwrap_or_else(|_| template.to_string())
    }
}